"""HTTP client for Ollama /api/chat (non-streaming) and /api/show (model metadata)."""
...
# Tokens reserved for system prompt, user text, and assistant reply (not scene digest).
...
def chat_completion(
    ...
    Streaming avoids Ollama's 'context canceled' that occurs with large non-streaming requests.
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Try a smaller model, lower scene context, or Stop and retry."
        ) from e
...
def show_model(base_url: str, model: str, timeout: float = 10.0) -> dict[str, Any]:
    """POST /api/show — model metadata including context length."""
    return _post_json(base_url, "/api/show", {"model": model}, timeout)


def parse_parameters_num_ctx(parameters: Any) -> Optional[int]:
    """Read num_ctx from Ollama's parameters field (string or mapping)."""
...
def parse_model_info_context_length(model_info: Any) -> Optional[int]:
    """Largest *.context_length value from model_info (e.g. llama.context_length)."""
    if not isinstance(model_info, dict):
        return None
    best: Optional[int] = None
    for key, val in model_info.items():
        key_s = str(key).lower()
        if "context_length" not in key_s:
            continue
...
def suggest_max_scene_chars(
    ...
    *,
    ...
    """Scene digest char budget from token context (rough chars/token estimate)."""
...
def resolve_context_settings(show_data: dict[str, Any]) -> dict[str, Any]:
    """
    Derive num_ctx and max_scene_chars from /api/show response.

    Uses the model's trained context_length when present; otherwise configured num_ctx.
    """
    ...
    if model_max and configured:
        ...
        return {
            "num_ctx": 0,
            "max_scene_chars": 48_000,
            "model_context_max": None,
            "configured_num_ctx": None,
            "summary": "No context length in /api/show; using Ollama runtime default and 48k scene chars.",
        }

    max_scene = suggest_max_scene_chars(num_ctx)
    return {
        "num_ctx": num_ctx,
        "max_scene_chars": max_scene,
        "model_context_max": model_max,
        "configured_num_ctx": configured,
        "summary": (
            f"Context {num_ctx:,} tokens ({source}); "
            f"scene digest ≤ {max_scene:,} chars (~{max_scene / _CHARS_PER_TOKEN:,.0f} tokens)."
        ),
    }


def get_model_context_settings(base_url: str, model: str, timeout: float = 10.0) -> dict[str, Any]:
    """Fetch /api/show and return recommended BlenderGPT context preferences."""
...
def resolve_model_name(base_url: str, model: str, timeout: float = 5.0) -> str:
    """Match configured model to an installed Ollama tag (e.g. llama3.2 → llama3.2:latest)."""
    model = (model or "").strip()
    if not model:
        raise RuntimeError("No model configured in add-on preferences.")
...
def _try_launch_ollama_host() -> bool:
    """Best-effort start of the Ollama app / service when nothing is listening yet."""
...
def wait_for_connection(
    ...
    *,
    ...
    """Poll /api/tags until Ollama responds or attempts are exhausted."""
    launched = False
...
def warm_model(
    ...
    *,
    ...
    """
    Load the model into memory with a tiny generate call (first real request is faster).
    """
    model = (model or "").strip()
    if not model:
        return
...
def ensure_ollama_ready(
    ...
    *,
    ...
    """
    Wake Ollama if needed (/api/tags), optionally preload the configured model.
    Raises RuntimeError when the server never becomes reachable.
    """
